...
export const FooterNew = () => {
    const serviceLinks = [
        { name: "Student Support", href: "/categories" },
        { name: "Job Opportunities", href: "/categories" },
        { name: "Document Services", href: "/categories" },
        { name: "Business Promotion", href: "/advertising" },
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
                    ...
                    <div className="lg:col-span-1 space-y-5">
                        ...
                        <p className="text-sm text-background/40 font-body leading-relaxed">
                            Empowering students, connecting opportunities, and building futures across Nigeria.
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <ul className="space-y-3">
                            {serviceLinks.map((link) => (
                                <li key={link.name}>
                                    <Link to={link.href} className="text-sm text-background/50 hover:text-primary transition-colors font-body">{link.name}</Link>
                                </li>
                            ))}
                        </ul>
                    </div>
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-5">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
                        ...
                        <div className="flex items-center gap-5">
                            {legalLinks.map((link) => (
                                <Link key={link.name} to={link.href} className="text-xs text-background/40 hover:text-primary transition-colors font-body">{link.name}</Link>
                            ))}
                        </div>
                    </div>
                </div>
