-- Catalog seed v3 (idempotent; safe in both environments)
...
VALUES
  ('starter','Starter',NULL,'Quick daily visibility — Morning & Evening promotion.',2000,'/day','1 Day Promotion',1,
    '["2 posts daily (Morning & Evening)","Quick and affordable visibility"]'::jsonb,false,true,true,10),
  ('weekly','Weekly',NULL,'Consistent weekly visibility for better reach.',10500,'','7 Days Promotion',7,
    '["Consistent daily promotion","Better reach and engagement"]'::jsonb,false,true,true,20),
  ('growth','Growth','🔥','Best value for business growth.',18200,'','14 Days Promotion',14,
    '["Extended promotion period","Strong audience reach","Higher engagement"]'::jsonb,true,true,true,30),
  ('premium','Premium','💎','Maximum visibility with priority placement.',36000,'','30 Days Promotion',30,
    '["Maximum visibility","Priority placement","Long-term promotion"]'::jsonb,false,true,true,40),
  ('promote-with-link','Promote with Link','🔗','Promote your business with a clickable link directing to your site or socials.',5000,'/day','1 Day Promotion with Link',1,
    '["Clickable promotional link","Direct traffic to your site or socials","Daily renewable placement"]'::jsonb,false,true,true,50)
ON CONFLICT (slug) DO UPDATE
...
INSERT INTO public.businesses
  (name,category,description,short_description,state,city,phone,verified,promotion_tier,status)
SELECT 'CAC REGISTRATION FOR BUSINESS','General','Active','','FCT - Abuja','Gana Street Abuja','07040294858',true,0,'approved'
WHERE NOT EXISTS (SELECT 1 FROM public.businesses WHERE status = 'approved');
